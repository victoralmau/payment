            ...
            if self.acquirer_id.done_sale_order_user_id_note==True:
                for sale_order_id in self.sale_order_ids:
                    mail_message_vals = {
                        ...
                        'record_name': sale_order_id.name                                                                        
                    }
                    #add_auto_starred
                    if sale_order_id.user_id.id>0:
                        mail_message_vals['starred_partner_ids'] = [[6, False, [sale_order_id.user_id.partner_id.id]]]
                    #create
                    if sale_order_id.user_id.id>0:
                        mail_message_obj = self.env['mail.message'].sudo(sale_order_id.user_id.id).create(mail_message_vals)
                    else:
